...
// get the lowest possible sum of the current cards, used to check if player
// has busted
int getMinSum(std::vector<int>);

// pre: must receive vector of possible sums of cards
// post: returns best possible sum (largest value <= 21) of the current
// set of cards
int getBest(std::vector<int>);

void printVec(std::vector<int>);

int sendStatus(int, int);
int getStatus(int *);
int getStatus(int);
